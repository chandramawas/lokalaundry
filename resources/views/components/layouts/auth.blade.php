...
<body class="bg-surface text-on-surface font-sans text-base antialiased cursor-default">
    <div class="min-h-screen grid grid-cols-1 md:grid-cols-2">
        ...
        {{-- Right --}}
        <div class="size-full px-6 md:px-12 py-10 flex flex-col gap-4 justify-center lg:justify-start">
            {{-- Session Message --}}
            @if (session('success'))
                <div x-data="{ show: true }" x-init="setTimeout(() => show = false, 5000)" x-show="show" x-cloak
                    x-transition class="p-2 bg-green-100 text-green-800 rounded-md text-sm text-center">
            ...
            @if (session('error'))
                <div x-data="{ show: true }" x-init="setTimeout(() => show = false, 5000)" x-show="show" x-cloak
                    x-transition class="p-2 bg-red-100 text-red-800 rounded-md text-sm text-center">
